...
BatchNorm ablation variant of train_cnn.py: identical architecture and
training config, with every GroupNorm(1, C) swapped for BatchNorm1d(C). This
mirrors the normalization choice used in prior DroneRF work (Al-Sa'd 2019,
Al-Emadi 2020) so we can test the hypothesis that BatchNorm-based classifiers
are more severely miscalibrated than the LayerNorm/GroupNorm variant used as
our main CNN baseline (train_cnn.py), which was adopted specifically because
BatchNorm was unstable on the sparse log-spectrum input. Everything else
(three conv blocks [32, 64, 128], Adam lr=3e-4, batch 128, 50 epochs, early
stopping patience=10 on val_loss, ReduceLROnPlateau factor=0.5 patience=4,
resumable checkpointing) is unchanged.
"""
import csv
import json
import sys
...
OUT_DIR = RESULTS_DIR / "baseline_cnn_bn"

torch.manual_seed(RANDOM_STATE)
np.random.seed(RANDOM_STATE)


class CNN1DBN(nn.Module):
...
def main():
    ...
    base = save_results("cnn_bn", y_test, y_pred, test_logits, train_time)
