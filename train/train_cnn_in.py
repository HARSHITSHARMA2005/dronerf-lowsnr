"""1D CNN control variant for DroneRF 4-class classification (PyTorch, CPU).

InstanceNorm control for the pooling-axis hypothesis: the BN-staleness
hypothesis is dead (results/diagnostics/bn_shift.json shows CNN-BN's
normalised statistics divergence stays below 0.5 at all tested SNRs, while
MLP-BN's raw drift is ~7x larger yet MLP-BN survives). New hypothesis: the
problem is the POOLING AXIS, not the stored statistics. BatchNorm1d in a conv
net pools per-channel across all 2048 frequency positions; the sparse
log-spectrum means an additive noise floor lifts thousands of low-power
positions that then dominate the per-channel pooled statistic, corrupting
normalisation for the informative high-power positions too.

InstanceNorm1d pools across positions exactly like BatchNorm (same axis) but
computes statistics per-sample at inference with NO stored running
statistics -- so if pooling-axis is the cause, CNN-IN should collapse like
CNN-BN; if staleness were the cause (already ruled out), CNN-IN should be
fine like GroupNorm.

Exact copy of train/train_cnn.py with every GroupNorm(1, C) replaced by
nn.InstanceNorm1d(C, affine=True, track_running_stats=False). Everything
else (three conv blocks [32, 64, 128], kernels 7/5/3, Adam lr=3e-4, batch
128, 50 epochs, early stopping patience=10 on val_loss, ReduceLROnPlateau
factor=0.5 patience=4, best-val-loss checkpoint, resumable checkpointing) is
unchanged.
"""
import csv
import json
...
OUT_DIR = RESULTS_DIR / "baseline_cnn_in"

torch.manual_seed(RANDOM_STATE)
np.random.seed(RANDOM_STATE)


class CNN1DIN(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES, channels=CHANNELS, dropout=DROPOUT):
        ...
        for i, out_ch in enumerate(channels):
            ...
            blocks.append(nn.InstanceNorm1d(out_ch, affine=True, track_running_stats=False))
...
def main():
    ...
    base = save_results("cnn_in", y_test, y_pred, test_logits, train_time)
