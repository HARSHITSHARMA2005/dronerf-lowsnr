...
BatchNorm ablation variant of train_mlp.py: identical architecture and
training config, with a BatchNorm1d layer inserted after each hidden Linear
(before ReLU/Dropout). Linear(2048, 512) -> BatchNorm1d(512) -> ReLU ->
Dropout(0.2) -> Linear(512, 128) -> BatchNorm1d(128) -> ReLU -> Dropout(0.2)
-> Linear(128, 4). This mirrors the normalization choice common in prior
DroneRF work (Al-Sa'd 2019, Al-Emadi 2020) so we can test whether BatchNorm
classifiers are more severely miscalibrated than the LayerNorm-free MLP used
as our main baseline (train_mlp.py). Training config mirrors train_mlp.py
exactly (Adam lr=3e-4, batch 128, 50 epochs, early stopping patience=10 on
val_loss, ReduceLROnPlateau factor=0.5 patience=4). Resumable checkpointing
protects against Windows killing the process mid-run.
"""
import csv
import json
import sys
...
OUT_DIR = RESULTS_DIR / "baseline_mlp_bn"
...
def main():
    ...
    base = save_results("mlp_bn", y_test, y_pred, test_logits, train_time)
