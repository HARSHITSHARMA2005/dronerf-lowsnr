"""CHECK A (multi-seed retraining) -- trains CNN-BN or CNN-IN with a given
random seed and saves to results/baseline_{model}_s{seed}/, so the BN-collapse
/ IN-robustness finding at 5 dB (CNN-BN 14.17%, CNN-IN 79.97%, seed 42) can be
checked against two more independent training runs before writing it up.

Data split is fixed (preprocess/load_and_split.py uses random_state=42
regardless of this script's --seed), so only model init + minibatch shuffle
order vary across seeds -- exactly reproducing the seed-42 baselines' training
protocol otherwise (3 conv blocks [32,64,128], Adam lr=3e-4, batch 128, 50
epochs, early stop patience=10 on val_loss, ReduceLROnPlateau factor=0.5
patience=4, best-val-loss checkpoint).

Imports CNN1DBN / CNN1DIN directly from train/train_cnn_bn.py and
train/train_cnn_in.py so the architecture is byte-identical to the seed-42
baselines; only the seed and output directory differ.

Usage:
  python diagnose\\train_multiseed.py --model cnn_bn --seed 43
  python diagnose\\train_multiseed.py --model cnn_bn --seed 44
  python diagnose\\train_multiseed.py --model cnn_in --seed 43
  python diagnose\\train_multiseed.py --model cnn_in --seed 44

Do NOT run this automatically -- the user runs it manually (each run is a
full 50-epoch CPU training, ~10-20 min based on the original seed-42 runs).
"""
import argparse
import csv
import json
...
from utils import load_splits, save_results, RESULTS_DIR  # noqa: E402
...
MODEL_REGISTRY = {
    "cnn_bn": (CNN1DBN, "cnn_bn"),
    "cnn_in": (CNN1DIN, "cnn_in"),
}
...
def main():
    ...
    model_class, model_tag = MODEL_REGISTRY[args.model]
    seed = args.seed
    out_name = f"baseline_{model_tag}_s{seed}"
    ...
    print(f"[{out_name}] Loaded splits: train={X_train.shape}, val={X_val.shape}, "
    ...
    base = save_results(out_name.replace("baseline_", ""), y_test, y_pred, test_logits, train_time)
